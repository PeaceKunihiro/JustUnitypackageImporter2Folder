...
「これでインポートする」は「参照」「クリア」の右隣に表示されます。UnityPackage、Import対象、Import先などの入力が揃ってImport可能になると、ボタンが有効化され青色で表示されます。
...
Default Import先とは別に、複数のImport先をプリセットとして保存できます。プリセット数に固定上限はありません。
...
「Import先プリセット」プルダウンから項目を選択すると、保存されたパスが現在のImport先へ即時反映され、「インポート先を変更する」がONになります。

「プリセットを管理」を展開すると、次の操作を行えます。

- 現在のImport先を新しいプリセットとして追加
- 表示名とインポート先パスを直接編集
- プリセットを削除
- `▲`と`▼`で並び順を変更

プリセットは`EditorUserSettings`へ保存され、プロジェクト単位で保持されます。Default Import先とは別管理のため、プリセットの追加・変更・削除によってDefault設定は変更されません。
...
展開後サイズは各TARエントリのヘッダーに記録されたサイズから、ペイロードを展開する前に累積します。累積値が設定した展開後サイズ警告値を初めて超えた時点で読み込みを一時停止し、次の確認を表示します。

> 展開後サイズが警告値 512 MB を超えています。
> 現在の推定展開後サイズ: 534.2 MB
> このままUnityPackageを読み込みますか？

「読み込みを続行」を選ぶと、そのPackageでは同じ確認を再表示せず解析を続けます。「キャンセル」を選ぶと解析を中断し、UnityPackage入力と読み込み途中の内容をクリアします。
