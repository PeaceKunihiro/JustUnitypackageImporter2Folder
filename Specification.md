...
Default Import先とは別に、複数のImport先プリセットを保持できるようにする。プリセット数に固定上限は設けない。
...
保存済みプリセットはプルダウンリストへ表示する。ユーザーがプリセットを選択した時点で、保存されたパスを現在のImport先へ即時反映し、「インポート先を変更する」をONにする。

以下の管理操作に対応する。

* 現在のImport先からプリセットを追加
* 表示名とインポート先パスを編集
* プリセットを削除
* プリセットを上または下へ移動して並び順を変更

プリセットは`EditorUserSettings`へJSON形式で保存し、JUI終了後もプロジェクト単位で保持する。

Default Import先は従来どおり独立した設定として保持し、プリセットの追加、編集、削除、並び順変更ではDefault設定を変更しない。
...
UnityPackage入力欄の「参照」「クリア」の右隣に、

**「これでインポートする」**

ボタンを設置する。

UnityPackageが読み込まれ、Import対象が1件以上選択され、Import先およびまとめ先フォルダ名が有効な場合をImport可能状態とする。

Import可能状態ではボタンを有効化して青色で表示する。それ以外の場合はボタンを無効化する。
...
UnityPackage
[Alice_v1.4.unitypackage] [参照] [クリア] [これでインポートする]
...
展開後サイズは、各TARエントリのヘッダーに記録されたサイズをペイロード読込前に累積する。

累積展開後サイズが設定した警告値を初めて上回った時点で、Package解析を一時停止して次の確認を表示する。

```text
展開後サイズが警告値 512 MB を超えています。
現在の推定展開後サイズ: 534.2 MB
このままUnityPackageを読み込みますか？

[読み込みを続行] [キャンセル]
```

「読み込みを続行」を選択した場合、そのPackageの解析中は同じサイズ確認を再表示せず読み込みを継続する。

「キャンセル」を選択した場合はペイロードを読み込まずに解析を中断し、UnityPackage入力および読み込み途中の内容をクリアする。Import先と保存済み設定は維持する。
...
更新履歴は [`UPDATELOG.md`](./UPDATELOG.md) を参照する。
